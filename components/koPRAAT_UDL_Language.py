# Registers the PRAAT language in Komodo.

import logging
from koUDLLanguageBase import KoUDLLanguage

log = logging.getLogger("koPRAATLanguage")
#log.setLevel(logging.DEBUG)

def registerLanguage(registry):
    log.debug("Registering language PRAAT")
    registry.registerLanguage(KoPRAATLanguage())

class KoPRAATLanguage(KoUDLLanguage):

    # ------------ Komodo Registration Information ------------ #

    name = "PRAAT"
    lexresLangName = "PRAAT"
    _reg_desc_ = "%s Language" % name
    _reg_contractid_ = "@activestate.com/koLanguage?language=%s;1" % name
    _reg_categories_ = [("komodo-language", name)]
    _reg_clsid_ = "144c2b75-b10f-4c0e-af9d-6ac4115716b0"

    defaultExtension = '.praat'
    ...
    #           'M': 'XML',             #   XML
    #        }
    lang_from_udl_family = {'SSL': 'PRAAT'}
