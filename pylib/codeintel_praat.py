...
lang = "PRAAT"
log = logging.getLogger("codeintel.praat")
#log.setLevel(logging.DEBUG)


# These keywords are copied from "PRAAT-mainlex.udl" - be sure to keep both
# of them in sync.
keywords = [
    # Keywords
        "if",
...
# lexers. You must define a Lexer class for lang PRAAT. If Komodo's
# scintilla lexer for PRAAT is UDL-based, then this is simply:
#
#   from codeintel2.udl import UDLLexer
#   class PRAATLexer(UDLLexer):
#       lang = lang
#
# Otherwise (the lexer for PRAAT is one of Komodo's existing C++ lexers
...
#   class PRAATLexer(Lexer):
#       lang = lang
#       def __init__(self):
#           self._properties = SilverCity.PropertySet()
#           self._lexer = SilverCity.find_lexer_module_by_id(ScintillaConstants.SCLEX_PRAAT)
#           self._keyword_lists = [
...
# examples of usage.
class PRAATLangIntel(LangIntel):
    lang = lang
...
# PRAAT content and returns a description of the code in that file.
# See "cile_praat.py" for more details.
#
# The CILE Driver is a class that calls this CILE. If PRAAT is
...
class PRAATCILEDriver(UDLCILEDriver):
    lang = lang

    def scan_purelang(self, buf):
        import cile_praat
        return cile_praat.scan_buf(buf)
...
def register(mgr):
    ...
    mgr.set_lang_info(
        lang,
        silvercity_lexer=PRAATLexer(),
        buf_class=PRAATBuffer,
        langintel_class=PRAATLangIntel,
        import_handler_class=None,
        cile_driver_class=PRAATCILEDriver,
