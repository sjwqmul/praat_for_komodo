...
"""A Code Intelligence Language Engine for the PRAAT language.
...
Module Usage:
    from cile_praat import scan
    mtime = os.stat("bar.praat")[stat.ST_MTIME]
    content = open("bar.praat", "r").read()
    scan(content, "bar.praat", mtime=mtime)
"""
...
class PRAATCILEError(CILEError):
    pass
...
log = logging.getLogger("cile.praat")
...
def scan_buf(buf, mtime=None, lang="PRAAT"):
    """Scan the given PRAATBuffer return an ElementTree (conforming
    to the CIX schema) giving a summary of its code elements.
    
    @param buf {PRAATBuffer} is the PRAAT buffer to scan
    ...
    """
    # Dev Notes:
    # - This stub implementation of the PRAAT CILE return an "empty"
    #   summary for the given content, i.e. CIX content that says "there
    #   are no code elements in this PRAAT content".
    # - Use the following command (in the extension source dir) to
    #   debug/test your scanner:
    #       codeintel scan -p -l PRAAT <example-PRAAT-file>
    ...
    # Dev Note:
    # This is where you process the PRAAT content and add CIX elements
    ...
    #   Use the "codeintel html -b <example-PRAAT-file>" command as
    #   a debugging tool.
    # - "buf.accessor.text" is the whole content of the file. If you have
    #   a separate tokenizer/scanner tool for PRAAT content, you may
    #   want to use it.
